...
startTime = 0
theLog = open('thelog.txt','w+')
theLog.write('output to file works')
...
def encode(imei, imsi , datetime, cpuID, latitude, longitude, alt, speed, rcount, temp, spare, spare2):
	...
	##setup output file
	outfilename = os.path.expanduser("/home/thepi/Desktop/o.txt") ##output file
	
	o = open(outfilename,"a")

	Tdate = datetime.split(' ')
	time = Tdate[1].split(':')
	date = Tdate[0].split('-')
	...
	if newPacket == 1:
		...
		Gspare2 = spare2
		newPacket = 0
		s = str(imei) + '&' + str(imsi) + '&' + str(date) + '&' + str(time) + '&' + str(cpuID) + '&' + str(latitude) + '&' + str(longitude) + '&' + str(alt) + '&' + str(speed) + '&' + str(rcount) + '&' + str(temp) + '&' + str(spare) + '&' + str(spare2) + '*'
	else:
		
		stime = int(Gtime) - int(time)
		sdate = int(Gdate) - int(date)
		slatitude = float(Glatitude) - float(latitude)
		slongitude = float(Glongitude) - float(longitude)
		salt = float(Galt) - float(alt)
		sspeed = float(Gspeed) - float(speed)
		srcount = int(Grcount) - int(rcount)
		stemp = float(Gtemp) - float(temp)
		sspare = int(Gspare) - int(spare)
		sspare2 = int(Gspare2) - int(spare2)
		s = str(sdate) + '&' + str(stime) + '&' + str(slatitude) + '&' + str(slongitude) + '&' + str(salt) + '&' + str(sspeed) + '&' + str(srcount) + '&' + str(stemp) + '&' + str(sspare) + '&' + str(sspare2) + '*'
            ...
            if matchcommand and response:
                if currentState == "GetIMSI" or currentState == "GetIMEI":
                    ...
                    if latitude and longitude:
                        ...
                        result = postJsonM2MServer(M2MserverUrl, latitude, longitude, timedateUTC, imei, imsi, cpuID, teamname, altitude,VelHoriz)
                        print result
                        time.sleep(DELAY)
                    else:
